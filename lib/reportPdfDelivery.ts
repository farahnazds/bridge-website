...
import { assembleCombinedMeasured, assembleMeasured } from "@/lib/reportPdf/assemble";
import { renderCombinedReportDocument, renderReportDocument } from "@/lib/reportPdf/render";
...
export interface StoreReportPdfInput {
   ...
   *
   * Optional on purpose: a caller that omits it (and passes no reportTypes)
   * falls straight through to the original renderer. Adding the field could
   * not break an existing call site even if one were missed.
   */
  reportType?: ReportType;
  /**
   * COMBINED reports only: the 2-3 selected domains, in selection order.
   * Mutually exclusive with reportType (reportType wins if both are set).
   * When present, the structured combined composer renders the document;
   * omitted, a combined report takes the legacy renderer as before.
   */
...
export async function generateAndStoreReportPdf(
  ...
  //
  // Combined reports (reportTypes, 2-3 domains) go through the structured
  // combined composer (lib/reportPdf/layouts/athleteCombined.ts) since
  // 2026-08-17; a call with NEITHER field still skips straight to the legacy
  // renderer rather than guessing.
  let bytes: Uint8Array | null = null;
  let fellBack: string | null = null;

  const combinedTypes =
    !input.reportType && input.reportTypes && input.reportTypes.length >= 2
      ? input.reportTypes
      : null;

  if (input.reportType || combinedTypes) {
    try {
      const measured = combinedTypes
        ? await assembleCombinedMeasured(
            combinedTypes,
            input.athleteId,
            input.periodStart,
            input.periodEnd,
      ...
      const footerNote = `Confidential — clinical record. Generated by ${input.generatedByName} on ${new Date()
        .toISOString()
        .slice(0, 10)}.`;
      bytes = combinedTypes
        ? await renderCombinedReportDocument({
            types: combinedTypes,
            identity,
            measured,
