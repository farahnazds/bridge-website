...
> **The code described in this section is NOT on `main`.** It lives on the
> branch `report-pdf-generator` (pushed to origin), deliberately unmerged and
> wired to nothing. Every path below — `lib/reportPdf/…`, `app/api/dev/…` —
> exists only there. `main` still runs the original `lib/reportPdf.ts`
> generator, unchanged.
>
> `git checkout report-pdf-generator` to see any of it.
